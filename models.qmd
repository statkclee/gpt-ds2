...
`predict`는 과학, 사회학, 심리학 등에서 사용되ㅕ 일반적으로 특정 사건이나 결과가 일어날 것이라고 말하는 데 사용된다. 예를 들어, 기계학습 모형이 특정 데이터를 바탕으로 결과를 '예측(predict)'하는 경우가 대표적이다. 특정 조건이나 상황에서 특정 결과가 일어날 것이라고 하는, 비교적 구체적이고 확정적인 예측을 내포한다.
